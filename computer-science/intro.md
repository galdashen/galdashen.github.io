...
第二步，打开 `C:\msys64\ucrt64.exe` 输入命令 `pacman -S mingw-w64-ucrt-x86_64-toolchain`
...
安装完成后可以开始写代码了，新建一个 cpp 文件，随便起个名字例如 `name1.cpp`，然后把以下代码复制进去
...
然后保存文件，打开终端至文件所在文件夹并运行命令 `g++ name1.cpp -o name2`，意思是使用 `g++` 编译器，将 `name1.cpp` 这个源代码文件，编译并链接成一个名为 `name2.exe` 的可执行程序。编译完成后，输入命令 `.\name2` 就可运行程序。
...
C/C++ 常用的数据类型包括整数 `int`，双精度浮点数 `double`，字符 `char` 等，这些数据类型都是以二进制的形式储存在内存里的，一个 `int` 占 4 个字节，一个 `double` 占 8 个字节，一个 `char` 占 1 个字节，所有的数据都是整数个字节倍的大小，计算机通常按字节给数据编址。可以把内存看成是一个字节一个字节的二进制数据按顺序排好，从 0 开始编址，通过地址就能找到数据在内存里的位置，从而读取它。指针里存的是地址，相当于指向了内存里的位置。我们可以手动设置一个指针，指定它的指向的数据类型，然后给它指向的地方分配一块连续的内存，这样可以在内存里创建一个数据，还可以通过把分配的内存设置为单个该数据类型大小的整数倍，创建一个数组。需要注意的是，指针的指向是分配的连续内存的最开头的那一个字节的位置。
